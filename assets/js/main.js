...
sv.add("(min-width: 1200px)", () => {
  let projectpanels = gsap.utils.toArray('.tp-service-panel');
  ...
  projectpanels.forEach((panel, index) => {
    ScrollTrigger.create({
      ...
      pin: true,
      pinSpacing: false,
      // Last card stays pinned, others unpin when next card reaches top
      endTrigger: index === projectpanels.length - 1 ? panel : projectpanels[index + 1],
      end: index === projectpanels.length - 1 ? "bottom top" : "top top",
      markers: false,
...
vd.add("(min-width: 1199px)", () => {
	...
	document.querySelectorAll(".tp_reveal_anim").forEach(areveal => {
		const duration = areveal.getAttribute("data-duration") || 0.2;
		const onScroll = areveal.getAttribute("data-on-scroll") || 0.1;
...
// Add this at the beginning of main.js or create a new file called 'fixes.js'
(function() {
    'use strict';
    
    // Safely initialize GSAP animations only if elements exist
    const originalGsapTo = window.gsap?.to;
    if (originalGsapTo) {
        window.gsap.to = function(targets, vars) {
            // Check if target exists before animating
            if (!targets || (typeof targets === 'string' && !document.querySelector(targets))) {
                console.warn('GSAP: Target not found, skipping animation');
                return;
            }
            return originalGsapTo.call(this, targets, vars);
        };
    }
})();